...
describe('FriendsService - Unit Tests', () => {
    ...
    beforeEach(() => {
        jest.clearAllMocks();
        friendsService = new friendsService_1.FriendsService();
    });
    describe('getFriends', () => {
    ...
    describe('inviteFriend', () => {
        it('should throw error when sender not found', async () => {
            ...
            await expect(friendsService.inviteFriend('non-existent', 'user2')).rejects.toThrow('User not found');
        });
        it('should throw error when trying to invite yourself', async () => {
            ...
            await expect(friendsService.inviteFriend('user1', 'user1')).rejects.toThrow('You cannot invite yourself');
        });
        it('should throw error when receiver not found', async () => {
            const mockPrisma = getMockPrisma();
            mockPrisma.user.findUnique
                .mockResolvedValueOnce({ id: 'user1', username: 'user1' })
                .mockResolvedValueOnce(null);
            await expect(friendsService.inviteFriend('user1', 'nonexistent')).rejects.toThrow('User with given username does not exist');
        });
        it('should throw error when already friends', async () => {
            ...
            await expect(friendsService.inviteFriend('user1', 'user2')).rejects.toThrow('You are already friends');
        });
        it('should throw error when invitation already exists', async () => {
            ...
            mockPrisma.friendInvite.findFirst.mockResolvedValue({
                id: 'invite1',
            });
            await expect(friendsService.inviteFriend('user1', 'user2')).rejects.toThrow('Invitation already sent or received');
        });
        it('should create invitation successfully', async () => {
            ...
            mockPrisma.friendship.findFirst.mockResolvedValueOnce(null);
            mockPrisma.friendInvite.findFirst.mockResolvedValueOnce(null);
            const mockInvite = {
                id: 'invite1',
                status: 'PENDING',
                createdAt: new Date(),
            ...
            mockPrisma.friendInvite.create.mockResolvedValue(mockInvite);
    ...
    describe('getInvites', () => {
        it('should return sent and received invites', async () => {
            ...
            mockPrisma.friendInvite.findMany.mockResolvedValue(mockInvites);
    ...
    describe('acceptInvite', () => {
        it('should throw error when invitation not found', async () => {
            const mockPrisma = getMockPrisma();
            mockPrisma.friendInvite.findUnique.mockResolvedValue(null);
            await expect(friendsService.acceptInvite('non-existent', 'user2')).rejects.toThrow('Invitation not found');
        });
        it('should throw error when user is not receiver', async () => {
            ...
            mockPrisma.friendInvite.findUnique.mockResolvedValue(mockInvite);
            await expect(friendsService.acceptInvite('invite1', 'user1')).rejects.toThrow('You do not have permission to accept this invitation');
        });
        it('should throw error when invitation is not pending', async () => {
            ...
            mockPrisma.friendInvite.findUnique.mockResolvedValue(mockInvite);
            await expect(friendsService.acceptInvite('invite1', 'user2')).rejects.toThrow('Invitation cannot be accepted');
        });
    ...
    describe('rejectInvite', () => {
        it('should throw error when invitation not found', async () => {
            const mockPrisma = getMockPrisma();
            mockPrisma.friendInvite.findUnique.mockResolvedValue(null);
            await expect(friendsService.rejectInvite('non-existent', 'user2')).rejects.toThrow('Invitation not found');
        });
        it('should reject invitation successfully', async () => {
            ...
            mockPrisma.friendInvite.findUnique.mockResolvedValue(mockInvite);
            mockPrisma.friendInvite.update.mockResolvedValue({
                ...mockInvite,
                status: 'REJECTED',
            });
    ...
    describe('deleteFriend', () => {
        it('should throw error when trying to remove yourself', async () => {
            await expect(friendsService.deleteFriend('user1', 'user1')).rejects.toThrow('You cannot remove yourself from the friend list');
        });
        it('should throw error when friendship not found', async () => {
            const mockPrisma = getMockPrisma();
            mockPrisma.friendship.findFirst.mockResolvedValue(null);
            await expect(friendsService.deleteFriend('user2', 'user1')).rejects.toThrow('Friendship not found');
        });
        it('should delete friend successfully', async () => {
            ...
            mockPrisma.friendship.findFirst.mockResolvedValue(mockFriendship);
...
//# sourceMappingURL=friendsService.test.js.map